...
O serviço `web` desta branch publica o site ACS em **https://acs.atcchinabrasil.com**. A imagem usa Node.js 24, pnpm 11.23.0 e o build `standalone` do Next.js, executado com usuário sem privilégios de root. O Dockerfile inclui uma verificação de saúde HTTP.

## Configuração

1. Na zona DNS de `atcchinabrasil.com`, crie um registro **A** para `acs` apontando para o IPv4 público do servidor Dokploy. As portas **80 e 443** devem estar acessíveis para o Traefik e a emissão do certificado.
2. Envie estes arquivos para o repositório usado no deploy.
3. No projeto do Dokploy, crie um serviço **Compose** separado do site ATC e selecione o tipo **Docker Compose**.
4. Conecte o repositório e selecione a branch `acs-group`. O serviço original da ATC deve continuar vinculado à `main`.
...
Os routers, middleware e serviço Traefik usam nomes próprios com o prefixo `acs-group`, mantendo o roteamento deste site separado do serviço ATC.

O Traefik existente recebe as conexões nas portas 80/443, redireciona HTTP para HTTPS e encaminha o tráfego pela rede Docker para a porta interna 3000, definida na label `traefik.http.services.acs-group.loadbalancer.server.port`. O Compose não usa `ports` ou `expose`, e o Dockerfile não declara `EXPOSE`; a porta 3000 não é publicada no host.
...
docker build -t acs-group:local .
docker run --rm --init -p 127.0.0.1:3000:3000 acs-group:local
```
